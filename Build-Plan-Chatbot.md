...
- Guardrails prompt/response results are asynchronously enriched from each request's Gateway log id and attached to the corresponding D1 user or assistant message with each category's individual action preserved. Insights labels flags as Gateway **request** or **response** flags and explicitly says `Full-context scan`, avoiding the false claim that a benign initiating message itself caused P1. Mixed results render separately (for example, `S9: BLOCK` and `P1: FLAG`). DLP match metadata comes from `cf-aig-dlp`/Gateway logs. Flags remain observability-only and never alter chatbot behavior or context.
...
The conversation-isolation foundation was deployed in version `cd82b6b8-d8bc-4a22-877e-b20f7c64a10a`; transcript/context separation was deployed in `903475b0-926a-4ed6-8599-9f995d92607b`; D1-backed per-session Insights was deployed in `afb5e831-80f9-4841-9570-5eb73fd0d57b`. D1 migrations `0001_chat_transcripts.sql` and `0002_guardrail_results.sql` are applied locally and remotely; the accurate full-context FLAG presentation is pending Worker deployment. Verification covers typecheck, JavaScript syntax, Wrangler dry run, guarded-then-benign and blocked-then-benign payload inspection, one-hour live-state behavior, duplicate-tab isolation, mixed per-category actions, customer-only transcript rendering, 30-day cascade cleanup, and summary compaction.
